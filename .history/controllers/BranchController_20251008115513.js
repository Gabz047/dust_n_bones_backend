...
class BranchController {
   ...
   static async getAll(req, res) {
        try {
            ...
            // Filtra por branch do contexto ou company do usuário
            if (req.context.branch?.id) {
                where.id = req.context.branch.id;
            } else if (req.context.company?.id) {
                where.companyId = req.context.company.id;
            }
            ...
            res.json({
                success: true,
                data: { branches: rows, pagination: { total: count, page, limit, totalPages: Math.ceil(count / limit) } }
            });
        } catch (error) {
